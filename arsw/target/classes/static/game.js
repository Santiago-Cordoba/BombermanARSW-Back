...
function loadGameMap() {
    fetch("http://localhost:8080/game/map")
        ...
        .then(map => {
            ...
            let mapContainer = document.getElementById("gameMapContainer");
            if (mapContainer) {
                mapContainer.innerText = map;
            } else {
                console.error("❌ Error: No se encontró el elemento 'gameMapContainer' en el HTML.");
            }
...
function drawMap(mapString) {
    ...
    const cols = Math.max(...rows.map(row => row.length)); // Obtener la fila más larga
    ...
    for (let y = 0; y < rows.length; y++) {
        for (let x = 0; x < rows[y].length; x++) {
            ...
            ctx.strokeRect(x * cellSize, y * cellSize, cellSize, cellSize); // Añadir bordes
        }
    }
}
